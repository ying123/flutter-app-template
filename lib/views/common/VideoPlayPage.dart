...
class VideoPlayerState extends State<VideoPlayerPage> {
  ...
  void initState() {
    super.initState();
    _listener = () {
      if (_controller.value.position.inMilliseconds >= _controller.value.duration.inMilliseconds && !_isShowPlay) {
        setState(() {
          _isShowPlay = true;
        });
      }
      else if (_controller.value.position.inMilliseconds < _controller.value.duration.inMilliseconds && _isShowPlay) {
        setState(() {
          _isShowPlay = false;
        });
      }
    ...
    setState(() {
      if (_controller.value.isPlaying) {
        if (_controller.value.position.inMilliseconds >= _controller.value.duration.inMilliseconds) {
          // todo: bug: 播放完成了，isPlaying还是true，
